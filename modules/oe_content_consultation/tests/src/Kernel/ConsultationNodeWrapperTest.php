...
class ConsultationNodeWrapperTest extends ConsultationKernelTestBase {
  ...
  public function testUnsupportedEntityType(): void {
    ...
    ConsultationNodeWrapper::getInstance($entity);
  }
  ...
  public function testUnsupportedEntityBundle(): void {
    $this->expectException(\InvalidArgumentException::class);
    $this->expectExceptionMessage("The current wrapper only accepts 'node' entities of type 'oe_consultation'.");
   ...
   *   Entity values.
   *
   * @return \Drupal\oe_content_consultation\ConsultationNodeWrapper
   *   Wrapper object.
   */
  protected function createWrapper(array $values): EntityWrapperInterface {
    ...
    $node->save();
    return ConsultationNodeWrapper::getInstance($node);
  }
