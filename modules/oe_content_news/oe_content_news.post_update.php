...
function oe_content_news_post_update_00003(): void {
  $storage = new FileStorage(\Drupal::service('extension.list.module')->getPath('oe_content_news') . '/config/post_updates/00003_create_fields');
...
function oe_content_news_post_update_00004(): void {
  $storage = new FileStorage(\Drupal::service('extension.list.module')->getPath('oe_content_news') . '/config/post_updates/00004_update_form_display');
...
function oe_content_news_post_update_20008(): void {
  ...
  $storage = new FileStorage(\Drupal::service('extension.list.module')->getPath('oe_content_news') . '/config/post_updates/20008_create_fields');
  \Drupal::service('config.installer')->installOptionalConfig($storage);
}
...
function oe_content_news_post_update_20009(): void {
  $storage = new FileStorage(\Drupal::service('extension.list.module')->getPath('oe_content_news') . '/config/post_updates/20009_update_form_display');
  $form_display_values = $storage->read('core.entity_form_display.node.oe_news.default');
  $form_display = EntityFormDisplay::load($form_display_values['id']);
  if ($form_display) {
    $updated_form_display = \Drupal::entityTypeManager()
